		...
		<div id="outbox">		
			...
			<div id="contents">
				<div id="member">
					...
					<form action="/member/join" name="jFrm" method="post">
					<div class="memberbd">
						<table summary="이름, 아이디, 비밀번호, 비밀번호 확인, 이메일, 이메일수신여부, 주소, 휴대폰, 유선전화, 생년월일 순으로 회원가입 정보를 등록할수 있습니다." class="memberWrite" border="1" cellspacing="0">
							...
							<tbody>
								<tr>
									...
									<td>
										<ul class="pta">
											...
											<li class="pt5"><span class="mvalign">한글만 입력가능</span></li>
										</ul>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>
										<ul class="pta">
											<li class="r10"><input type="password" id="pw2" name="pw2" class="w134" onkeyup="pwkey()"/></li>
											<li id="pwck">
											</li>
										</ul>
									</td>
								...
								<tr>
									...
									<td>
										<ul class="pta">
											<li><input type="text" id="eamil" name="eamil" class="w134" /></li>
											<li><span class="valign">&nbsp;@&nbsp;</span></li>
											<li class="r10"><input type="text" id="dMain" class="w134" /></li>
											<li>
												<select id="emailList" onchange="emBtn()">
								...
								<tr>
									...
									<td>
										<ul class="pta">
											<li>
												<input type="text" id="zCode" class="w134" />&nbsp;
											</li>
											<li><a onclick="zipBtn()" class="addressBtn"><span>우편번호 찾기</span></a></li>
											<li class="pt5"><input type="text" id="address1"class="addressType" /></li>
											<li class="pt5"><input type="text" id="address2"class="addressType" value="상세주소 입력" /></li>
											<li class="cb">
...
<script>
function sBtn(){
	// 이름, 아이디, 비밀번호/확인, 이메일, 전화번호 조건주기
	let nameCheck = /^[가-힣]+$/;
	let pwCheck = /^[a-zA-Z0-9]{4,20}$/;
	let idCheck = /^[a-zA-Z]{1}[a-zA-Z0-9_]{3,15}$/;
	let emailCheck = /^[a-zA-Z0-9@-_.]+$/;
	
	if(! idCheck.test($("#id").val()) ){
		alert("첫 글자는 영문으로 4~16자 까지 가능, 영문, 숫자와 특수기호(_)만 사용 가능");
		$("#id").focus();
		return false;
	}// if id
	...
	if(! pwCheck.test($("#pw1").val) ){
		alert("영문 / 숫자 혼용으로 4~20자 까지 가능");
		$("#pw").val("");
		$("#pw").focus();
		return false;
	} // if pw
...
function pwkey(){
	if($("#pw1").val() == $("#pw2").val()){
		$("#pwck").html('<span class="mvalign black">* 비밀번호가 일치합니다.</span>');
	}else{
...
function emBtn(){
	...
	if($("#emailList").val()=="#"){
		$("#dMain").val("");
		$("#dMain").attr("disabled", false);
	} else {
		$("#dMain").val($("#emailList").val());
		$("#dMain").attr("disabled", true);
	}
					...
					<div class="btnArea">
						<div class="bCenter">
							<ul>
								...
								<li><a onclick="sBtn()" class="sbtnMini">가입하기</a></li>
							</ul>
						</div>
					</div>
					</form>
